...
public class CashFlowBot
{
    ...
    private static readonly IDataBase DataBase = new SQLiteDataBase(Logger);
    ...
    //private static IStage GetCurrentStage(IUser user, string userName)
    //{
    //    if (!user.Exists)
    //    {
    //        user.Create();
    //        user.Name = userName;
    //        return new Start(user, Terms);
    //        //return new AskLanguage(user, Terms);
    //    }

    //    //if (!user.Person.Exists)
    //    //{
    //    //    return new AskProfession(user, Terms);
    //    //}

    //    //var stage = Type.GetType(user.StageName);
    //    var stage = Type.GetType($"CashFlowBot.Stages.{user.StageName}");
    //    return stage is not null
    //        ? (IStage) Activator.CreateInstance(stage, user, Terms)
    //        : throw new Exception($"{stage} stage not found!");
    //}

    //private static async void BotOnMessageReceived_OLD(object sender, MessageEventArgs messageEventArgs)
    //{
    //    try
    //    {
