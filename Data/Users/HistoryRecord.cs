        ...
        get
        {
            switch (Action)
            {
                case ActionType.PayMoney:
                    return Terms.Get(103, User.Id, "Pay {0}", Value.AsCurrency());

                case ActionType.GetMoney:
                    return Terms.Get(104, User.Id, "Get {0}", Value.AsCurrency());

                case ActionType.Child:
                    return Terms.Get(105, User.Id, "Get a child");

                case ActionType.Downsize:
                    return Terms.Get(106, User.Id, "Downsize and paying {0}", Value.AsCurrency());

                case ActionType.Credit:
                    return Terms.Get(107, User.Id, "Get credit: {0}", Value.AsCurrency());

                case ActionType.Charity:
                    return Terms.Get(108, User.Id, "Charity: {0}", Value.AsCurrency());
                ...
                case ActionType.BankruptcyBankLoan:
                    var reduceLiabilities = Terms.Get(40, User.Id, "Reduce Liabilities");
                    var type = Terms.Get((int)Action, User.Id, "Liability");
                ...
                case ActionType.BuyCoins:
                    var buyAsset = Terms.Get((int)Action, User.Id, "Buy Asset");
                    return $"{buyAsset}. {Asset.Description}";

                case ActionType.IncreaseCashFlow:
                    var increaseCashFlow = Terms.Get((int)Action, User.Id, "Increase Cash Flow");
