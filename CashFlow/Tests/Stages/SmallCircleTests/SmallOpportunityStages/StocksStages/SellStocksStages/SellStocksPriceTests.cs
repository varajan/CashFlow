...
public class SellStocksPriceTests : StagesBaseTest
{
    ...
    [Test]
    public async Task SellStocksPrice_SelectInvalidPrice_StayOnStage([Values("-1", "0", "$0", "test")] string price)
    {
        // Arrange
        var testStage = GetTestStage();
    ...
    public async Task SellStocksPrice_SelectValidName_MoveForward(string price)
    {
        // Arrange
        var testStage = GetTestStage();
        ...
        PersonManagerMock.Verify(p => p.Update(
            It.Is<PersonDto>(x =>
                x.Id == TestPerson.Id &&
                x.Cash == TestPerson.Cash + Assets.Where(a => a.MarkedToSell).Sum(a => a.Qtty) * price.AsCurrency())
        ), Times.Once);
