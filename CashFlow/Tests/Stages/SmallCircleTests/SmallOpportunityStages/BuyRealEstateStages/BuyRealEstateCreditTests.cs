...
public class BuyRealEstateCreditTests : StagesBaseTest
{
    ...
    private AssetDto Asset => new() { Id = 123, UserId = CurrentUserMock.Object.Id, Price = 10_000, Mortgage = 8_500, Type = AssetType.RealEstate, IsDraft = true };

    [SetUp]
    public void Setup()
    {
        AssetManagerMock.Setup(a => a.ReadAll(AssetType.RealEstate, TestPerson.Id)).Returns([Asset]);
    ...
    [Test]
    public async Task BuyRealEstateCredit_ToBank_Confirmed_IsCompleted()
    {
        // Arrange
        var firstPayment = Asset.Price - Asset.Mortgage;
