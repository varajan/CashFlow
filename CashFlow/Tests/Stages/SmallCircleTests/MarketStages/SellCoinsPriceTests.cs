...
[TestFixture]
public class SellCoinsPriceTests : SellAssetBaseTest
{
    ...
    [Test]
    public void SellCoinsPrice_Question_and_Buttons()
    {
        // Arrange
    ...
    [Test]
    public async Task SellCoinsPrice_CanBeCanceled()
    {
        // Arrange
        var testStage = GetTestStage();
    ...
    [Test]
    public async Task SellCoinsPrice_SelectInvalidPrice_StayOnStage([Values("-1", "0", "$0", "test")] string price)
    {
        // Arrange
        var testStage = GetTestStage();
    ...
    public async Task SellCoinsPrice_SelectValidValue_Completed(string price)
    {
        // Arrange
        var testStage = GetTestStage();
        var payedAmmount = 0;
        ...
        Assets
            ...
            .ForEach(asset =>
            {
                ...
                PersonServiceMock.Verify(a => a.SellAsset(asset, ActionType.SellCoins, price.AsCurrency(), CurrentUser), Times.Once);
                PersonServiceMock.Verify(x => x.AddHistory(ActionType.SellCoins, price.AsCurrency(), CurrentUser, asset.Id), Times.Once);
            });
    ...
    protected override IStage GetTestStage() => GetStage<SellCoinsPrice>();
}
