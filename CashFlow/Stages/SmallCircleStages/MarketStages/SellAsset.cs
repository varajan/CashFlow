...
public abstract class SellAsset<TNextStage>(
    ...
    : BaseStage(termsService, userService, personManager, userRepository) where TNextStage : BaseStage
{
    ...
    private List<AssetDto> Assets => AssetTypes.SelectMany(type => PersonService.ReadActiveAssets(type, CurrentUser)).ToList();
