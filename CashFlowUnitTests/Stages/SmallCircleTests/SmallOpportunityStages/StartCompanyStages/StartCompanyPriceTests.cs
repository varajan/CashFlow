...
{
    private static readonly string[] CompanyPrices = Prices.SmallBusinessBuyPrice.AsCurrency().ToArray();// ["$100", "$500"];
    ...
    public async Task StartCompanyPrice_SelectValidCount_MoveForward(int cash, int price, bool creditIsNeeded)
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            PersonServiceMock.Verify(x => x.AddHistory(ActionType.StartCompany,
                price,
                It.IsAny<UserDto>(),
    ...
    protected override IStage GetTestStage() => GetStage<StartCompanyPrice>();
}
