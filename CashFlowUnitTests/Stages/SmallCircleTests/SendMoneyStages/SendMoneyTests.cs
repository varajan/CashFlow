...
public class SendMoneyTests : StagesBaseTest
{
    ...
    [SetUp]
    public void Setup() => PersonServiceMock.Setup(a => a.ReadActiveAssets(AssetType.Transfer, CurrentUser)).Returns([Asset]);
    ...
    public void SendMoney_NoOtherUsers_CanSendToBankOnly()
    {
        ...
        {
            Assert.That(testStage.Message, Is.EqualTo("Whom?"));
            Assert.That(testStage.OLD_Buttons, Is.EqualTo(new List<string> { "Bank", "Cancel" }));
        }
    }
